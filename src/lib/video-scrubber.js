...
function resizeAdjustments() { // fit everything to the screen
    $('.service-header').css('height', (totalFrames * 30) + 'px'); // increase the height of the document 30 pixels for every frame in the JPEG sequence
    var image_width = $('#video').css('max-width').replace('px', '');
    var image_height = $('#video').css('min-height').replace('px', '');
    ...
    if (height_ratio < width_ratio) {
        ...
        $('#video').css('left', 0);
        var difference = parseInt(image_height - document.getElementsByClassName("service-header").clientHeight); // calculate the difference we need to accomodate for
        if (document.getElementsByClassName("service-header").clientHeight < image_height) {
